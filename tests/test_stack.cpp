...
TEST(s21_stack, empty_constructor) {
  s21::stack<int> s;
  // ASSERT_EQ(0, s.size());
  ASSERT_EQ(0, static_cast<int>(s.size()));
  ASSERT_TRUE(s.empty());
}
...
TEST(s21_stack, initializer_list_constructor) {
  ...
  // ASSERT_EQ(5, s.size());
  ASSERT_EQ(5, static_cast<int>(s.size()));
...
TEST(s21_stack, copy_constructor) {
  ...
  s21::stack<int> s2(s1);
  // ASSERT_EQ(5, s2.size());
  ASSERT_EQ(5, static_cast<int>(s2.size()));
...
TEST(s21_stack, size) {
  ...
  s.push(1);
  // ASSERT_EQ(1, s.size());
  ASSERT_EQ(1, static_cast<int>(s.size()));
}
...
TEST(s21_stack, push) {
  ...
  s.push(2);
  // ASSERT_EQ(2, s.size());
  ASSERT_EQ(2, static_cast<int>(s.size()));
  ASSERT_EQ(2, s.top());
}
...
TEST(s21_stack, pop) {
  ...
  s.pop();
  // ASSERT_EQ(4, s.size());
  ASSERT_EQ(4, static_cast<int>(s.size()));
  ASSERT_EQ(4, s.top());
  s.pop();
  // ASSERT_EQ(3, s.size());
  ASSERT_EQ(3, static_cast<int>(s.size()));
  ASSERT_EQ(3, s.top());
}
...
TEST(s21_stack, swap) {
  ...
  s1.swap(s2);
  // ASSERT_EQ(5, s1.size());
  ASSERT_EQ(5, static_cast<int>(s1.size()));
  ASSERT_EQ(10, s1.top());
  // ASSERT_EQ(5, s2.size());
  ASSERT_EQ(5, static_cast<int>(s2.size()));
  ASSERT_EQ(5, s2.top());
